...
typedef struct cm_rtpbcast_stats {
	...
	gdouble average_loss_rate;
	guint64 average_loss_count;
	gdouble current_loss_rate;
	guint64 current_loss_count;
...
static void cm_rtpbcast_stats_restart(cm_rtpbcast_stats *st) {
	...
	st->cur = -1.0;
	st->average_loss_rate = -1.0;
	st->average_loss_count = -1.0;
	st->current_loss_rate = -1.0;
	st->current_loss_count = -1.0;
...
static void cm_rtpbcast_stats_update(cm_rtpbcast_stats *st, gsize bytes, guint32 seq, int isvideo) {
	...
	if (ml - st->last_avg_usec >= cm_rtpbcast_settings.mountpoint_info_interval * STAT_SECOND) {
		...
		if (isvideo != -1) {
			...
			if (den != 0 && st->packets_since_last_avg < den)
				st->current_loss_rate = 1.0 - (gdouble)st->packets_since_last_avg / (gdouble) den;
			else
				st->current_loss_rate = 0.0;

			st->current_loss_count += den;
	...
	if (isvideo != -1) {
		...
		if (den != 0 && st->packets_since_start < den)
			st->average_loss_rate = 1.0 - (gdouble)st->packets_since_start / (gdouble) den;
		else
			st->average_loss_rate = 0.0;

		st->average_loss_count += den;
		/* If debug is enabled, output CSV for stat collection */
		JANUS_LOG(LOG_DBG, "%d, %s, %.6f, %.6f\n", janus_get_monotonic_time(), av_names[isvideo], st->current_loss_rate, st->average_loss_rate);
	}
...
json_t *cm_rtpbcast_source_stats_to_json(cm_rtpbcast_rtp_source *src) {
	...
	for (j = AUDIO; j <= VIDEO; j++) {
		...
		json_object_set_new(v, "packet-loss", (src->stats[j].current_loss_rate == -1) ? json_null() : json_real(src->stats[j].current_loss_rate));
		json_object_set_new(v, "packet-loss-count", (src->stats[j].current_loss_count == -1) ? json_null() : json_integer(src->stats[j].current_loss_count));
